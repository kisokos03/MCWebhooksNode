...
function initMCWebhookJSON() {
    ...
    fs.writeFileSync(".\\plugins\\webhooks\\MCWebhooks.json", JSON.stringify(InitJSON, null, 4));
...
function initialize(members, fileInit) {
    ...
    var webhookData = JSON.parse(fs.readFileSync(".\\plugins\\webhooks\\MCWebhooks.json"));
    wc.send({ embeds: [(statusEmbedGenerator({ title: "The server is currently ONLINE", color: "00cc00" }))] }).then(msg => {
        ...
        fs.writeFileSync(".\\plugins\\webhooks\\MCWebhooks.json", JSON.stringify(webhookData, null, "  "));
    });
...
function statusON() {
    ...
    const wc = new WebhookClient({ url: webhookURL });
    var statusMessageID = JSON.parse(fs.readFileSync(".\\plugins\\webhooks\\MCWebhooks.json")).statusMessageID;
    wc.editMessage(statusMessageID, { embeds: [embed] });
    embed = statusEmbedGenerator({ title: "Noone's joined yet", color: '212e44' })
    var membersMessageID = JSON.parse(fs.readFileSync(".\\plugins\\webhooks\\MCWebhooks.json")).membersMessageID;
...
function membersUpdate(members) {
    if (JSON.parse(fs.readFileSync('.\\plugins\\webhooks\\MCWebhooks.json')).status == '1') {
        ...
        const wc = new WebhookClient({ url: webhookURL })
        var membersMessageID = JSON.parse(fs.readFileSync('.\\plugins\\webhooks\\MCWebhooks.json'))
            .membersMessageID
        wc.editMessage(membersMessageID, { embeds: [embed] })
    }
}
